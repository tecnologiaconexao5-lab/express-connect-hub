...
import { Download, Printer, RefreshCw, FileText, DollarSign, TrendingUp } from "lucide-react";
...
export default function RelatorioFaturamentoCliente() {
    ...
    try {
      ...
      if (filtros.cliente) {
        filtrados = filtrados.filter((os: any) => 
          (os.cliente_id || "").toLowerCase().includes(filtros.cliente.toLowerCase())
        );
      }
  ...
  const totais = {
    os: dados.length,
    faturamento: dados.reduce((a, b) => a + (b.valor_total || 0), 0),
    ticketMedio: dados.length > 0 ? dados.reduce((a, b) => a + (b.valor_total || 0), 0) / dados.length : 0
  };
  ...
  const handleExportPDF = () => {
    ...
    gerarPDFProfissional({
      ...
      colunas: ["OS", "Data", "Cliente", "Status", "Valor (R$)"],
      linhas: dados.map(d => [
        d.numero || d.id.slice(0, 8),
        fmtData(d.created_at),
        d.cliente_id || "-",
        d.status || "-",
        (d.valor_total || 0).toFixed(2)
      ]),
      totais: [
        { label: "Total de OS:", valor: totais.os.toString() },
        { label: "Faturamento Total:", valor: fmtBRL(totais.faturamento) },
  ...
  const handleExportCSV = () => {
    if (dados.length === 0) return toast.error("Sem dados para exportar");
    const header = "OS;Data;Cliente;Status;Valor\n";
    const rows = dados.map(d => `${d.numero || d.id.slice(0, 8)};${fmtData(d.created_at)};${d.cliente_id || "-"};${d.status || "-"};${(d.valor_total || 0).toFixed(2)}`).join("\n");
    const blob = new Blob(["\ufeff" + header + rows], { type: "text/csv;charset=utf-8;" });
    const link = document.createElement("a");
    link.href = URL.createObjectURL(blob);
    link.download = "faturamento_cliente.csv";
    link.click();
      ...
      <Card className="bg-card border-border shadow-sm">
        <CardContent className="p-4 flex items-end gap-4">
          ...
          <div className="space-y-1 flex-1">
            ...
            <Input placeholder="Buscar cliente..." value={filtros.cliente} onChange={e => setFiltros({...filtros, cliente: e.target.value})} className="h-9" />
          </div>
        ...
        <Card className="border-border shadow-sm border-l-4 border-l-blue-500">
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">{fmtBRL(totais.faturamento)}</p>
            </div>
            <div className="p-3 bg-muted/50 rounded-full"><DollarSign className="w-6 h-6 text-blue-500" /></div>
          </CardContent>
        </Card>
        <Card className="border-border shadow-sm border-l-4 border-l-emerald-500">
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Total OS</p>
              <p className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">{totais.os}</p>
            </div>
            <div className="p-3 bg-muted/50 rounded-full"><FileText className="w-6 h-6 text-emerald-500" /></div>
          </CardContent>
        </Card>
        <Card className="border-border shadow-sm border-l-4 border-l-violet-500">
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-violet-600 dark:text-violet-400">{fmtBRL(totais.ticketMedio)}</p>
            </div>
            <div className="p-3 bg-muted/50 rounded-full"><TrendingUp className="w-6 h-6 text-violet-500" /></div>
          </CardContent>
        </Card>
      </div>
          ...
          <Table>
            <TableHeader className="bg-muted/30">
              <TableRow>
                ...
                <TableHead className="font-semibold text-xs uppercase">Data</TableHead>
                <TableHead className="font-semibold text-xs uppercase">Cliente</TableHead>
              ...
              {loading ? (
                ...
                dados.map(d => (
                  <TableRow key={d.id} className="hover:bg-muted/40">
                    ...
                    <TableCell className="text-xs max-w-[200px] truncate">{d.cliente_id || "-"}</TableCell>
                    <TableCell className="text-xs font-semibold text-right text-emerald-600 dark:text-emerald-400">{fmtBRL(d.valor_total || 0)}</TableCell>
                  </TableRow>
                ))
              )}
            </TableBody>
